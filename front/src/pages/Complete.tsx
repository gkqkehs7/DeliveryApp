...
function Complete() {
  ...
  //원래 저기 인자로 넣어야하는데 hook으로 한거 이렇게 안하면 타입따로명시
  ...
  //imgPicker로 찍은 이미지를 img Resizer에다가 넣어서 결과를(setImage)에 저장
  ...
  const onTakePhoto = useCallback(() => {
    return ImagePicker.openCamera({
      //카메라 켜기
      includeBase64: true, //이게 있어야 미리보기 표시할 수 있음
      includeExif: true, //사진방향자동 감지
      saveToPhotos: true,
      // cropping:true 사진찍고 나서 편집가능
    })
      .then(onResponse)
      .catch(console.log);
  }, [onResponse]);
